...
-- ============================================================
-- Bookkeeping dashboard (pages/other-tables.html)
-- ============================================================
-- This section pulls the numbers used by the "Bookkeeping" pages:
...
--
-- Output matches the shape expected by pages/other-tables.html’s charts:
...
WITH constants AS (SELECT 276::numeric                                                   AS donut_circumference,
     ...
     revenue_by_week AS (SELECT date_trunc('week', o.created_at)::date AS week_start,
                         ...
                         GROUP BY 1
                         ORDER BY 1),
     revenue_norm AS (SELECT r.week_start,
                             r.total,
                             NULLIF(MAX(r.total) OVER (), 0) AS max_total
                      FROM revenue_by_week r),
     expense_mtd AS (SELECT e.category,
     ...
     expense_geom AS (SELECT s.category,
                             s.total,
...
SELECT json_build_object(
               'revenueByWeek', (SELECT json_agg(
                                                json_build_object(
                                                        'week_start', week_start,
                                                        'total', total,
                                                        'v', CASE
                                                                 WHEN max_total IS NULL THEN 0
                                                                 ELSE ROUND(100 * total / max_total)::int END,
                                                        'label', to_char(week_start, 'Mon DD')
                                                )
                                                ORDER BY week_start
                                        )
                                 FROM revenue_norm),
               'expenseSplit', (SELECT json_agg(
                                               json_build_object(
                                                       'category', category,
                                                       'total', total,
                                                       'pct', pct,
